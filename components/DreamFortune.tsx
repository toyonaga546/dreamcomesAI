...
function detectTheme(text: string): Theme {
  ...
  const loveWords = ["好き", "彼氏", "彼女", "告白", "デート", "恋", "キス", "手をつなぐ","ときめき","片思い","両思い","愛情","ドキドキ",
"切ない","嫉妬","安心感","ときめく","惹かれる","ハグ","カップル","パートナー","恋人","意中の人","好きな人","プロポーズ","誘う","会話が弾む",
"気遣い","褒める","甘える","プロポーズ","見つめ合う","ボディタッチ","イルミネーション","花火大会",
"記念日","デートスポット","初恋", "再会", "復縁", "失恋", "別れ", "浮気", "三角関係", "片思い", "両思い", "恋愛成就",];
  const workWords = [
    "仕事","シフト", "社員","上司","同僚","研修","勤務時間","時給","労働時間","休憩","残業",
    "仕事量","職場","面接","マニュアル","出勤","退勤","アルバイト","講義", "教室", "出席", "ノート", 
    "発表", "教科書", "先生", "グループワーク", "休講", "必修科目", "単位", "勉強計画", "暗記", 
    "問題集", "過去問", "筆記試験", "小テスト", "模試", "点数", "合格", "不合格", "試験範囲", 
    "提出", "下書き", "文献調査", "参考文献", "形式", "ワード数", "校正", "添削", "期日", "タイムマネジメント",
     "徹夜", "進捗","バイト","テスト","試験","授業","レポート","勉強","課題","締切","成績","単位","卒業","就職",
     "受験","合格","不合格","面接","エントリーシート","志望動機","履歴書","推薦状","オープンキャンパス",
     "模擬試験","予備校","参考書","赤本","センター試験","共通テスト","小論文","筆記試験","グループディスカッション",
  ];
  const relationWords = [
    "友達","親友", "相棒", "知り合い", "仲間", "部活仲間", "同僚", "先輩", "指導", "後輩指導", "同期",
    "兄弟", "姉", "姉妹", "両親", "祖父", "祖母", "親戚", "従兄弟", "従姉妹", "家族", "家庭",
    "保護者", "クラス", "学年", "同学年", "同窓生", "幼馴染", "仲良し", "相性","友人",
    "先輩","後輩","家族","兄","弟","妹","父","母","同級生","クラスメイト","ルームメイト","チームメイト",
    "サークル仲間","先生","コーチ","メンター","マネージャー","顧客","取引先","隣人","近所付き合い","ママ友",
    "パパ友","マブダチ","腐れ縁","監督","お隣さん"
  ];
  const selfWords = ["自分", "将来", "一人", "ひとり", "鏡", "自分の顔", "成長", "変わった",
    "変化", "挑戦", "決断", "選択", "選ぶ","理想", "目標", "努力", "自信", "不安",
    "焦り", "迷い", "葛藤", "自己啓発", "内省", "反省", "メンタル", "心の声", "価値観", "人生観", 
    "哲学", "存在意義", "アイデンティティ", "自我","自己", "内面", "性格", "価値観", "目標", "夢", 
    "進路", "未来像", "孤独", "自立", "アイデンティティ", "セルフイメージ", "容姿", "見た目", 
    "自撮り", "姿勢","成長","自己理解", "改善", "節目",];
...
function detectSentiment(text: string): Sentiment {
  const positiveWords = ["楽しい", "嬉しい", "わくわく", "安心", "幸せ", "面白い", "ほっとした","喜び", 
    "満足", "爽快", "にこにこ", "笑顔", "快感", "心地よい", "リラックス", "癒し", "安堵", "ウキウキ", "ハッピー",
     "幸福感", "感動", "ほのぼの", "充実", "達成感", "楽観的", "前向き", "テンションが上がる","ポジティブ", 
     "やる気", "励みになる", "希望", "成功","たのしい","うれしい"];
  const negativeWords = ["怖い", "不安", "焦る", "疲れ", "つらい", "嫌", "怒る", "怒られた", "落ちる","恐怖",
     "怯える", "びくびく", "心配", "動揺", "パニック", "焦燥感", "緊張", "ストレス", "疲労", "限界", "しんどい", 
     "苦しい", "憂鬱", "嫌悪感", "イライラ", "不機嫌", "叱責", "注意された", "ショック", "失望", "落ち込む", 
     "どんより", "絶望", "ネガティブ",];
...
function detectScene(text: string): Scene {
  ...
  const homeWords = ["家", "部屋", "自分の部屋", "リビング", "実家", "マンション","マイホーム",
     "住まい", "自宅", "寝室", "キッチン", "ダイニング", "廊下", "玄関", "ベランダ", "家具", 
     "インテリア", "ワンルーム", "アパート", "集合住宅", "一軒家", "家庭", "家族の部屋", "プライベート空間",];
  const outsideWords = ["道", "道路", "駅", "公園", "空", "海", "山", "街中", "外","舗道", "横断歩道",
     "信号機", "バス停", "ホーム", "線路", "改札", "ベンチ", "広場", "遊歩道", "青空", "雲", "夕日", 
     "夜空", "波", "砂浜", "川辺", "森林", "丘", "街角", "繁華街", "屋外", "外気",
     "観光地", "ランドマーク", "公共の場", "オープンスペース",];
...
function detectTimeFrame(text: string): TimeFrame {
  ...
  const pastWords = [
    "昔","思い出", "記憶", "懐かしい", "幼少期", "青春時代", "成長期", "当時", "昔話", 
    "思い返す", "振り返り", "歴史","旧友", "初恋", "別れた相手", "元恋人", "元パートナー",
    "思い出の場所","過去","子供のころ","子どものころ","小さい頃","小さいころ","前に","以前",
    "あの頃","元彼","元カノ","元カレ","モトカノ","モトカレ","前の","まえの"
  ];
  const futureWords = [
    "将来","未来", "先のこと", "予定", "計画", "スケジュール", "見通し", 
    "将来像", "目標", "夢", "理想", "希望", "期待", "成長後", "社会人生活", 
    "新生活", "スタート", "節目", "タイミング", "そのうち", "長い目で見て",
"これから","来週","来月","来年","卒業後","就職してから","いつか",
  ];
...
function detectRole(text: string): Role {
  ...
  const activeWords = [
    "話しかけた","告白した","走った","追いかけた","助けた",
    "決めた","選んだ","飛び込んだ","挑戦した","声をかけた","打ち明けた",
"駆けた","追随した","救った","決断した","選択した","飛び出した","挑みかかった",
"おいかけた","たすけた","きめた","えらんだ","こえをかけた","うちあけた",
"かけた","助けた","たすけた",
  ];
  const passiveWords = [
    "追いかけられた","怒られた","襲われた","巻き込まれた","見ているだけだった",
    "何もできなかった","立ち尽くしていた","追われた","叱られた","攻撃された",
"巻き添えになった","見守るだけだった","手出しできなかった","立ちすくんでいた","おいかけられた",
"おこられた","おそわれた","まきこまれた","みているだけだった","なにもできなかった","おわれた",
"みまもるだけだった","てだしできなかった","たちすくんでいた",
  ];
